...
def plot_3d(ax, bones, output, color="red"):
	for p in output:
		if color == "blue":
			...
			ax.scatter(p[0], p[1], p[2], c='g', alpha=0.8)
		else:
	...
	for index in bones:
		...
		if color == "black":
			ax.plot(xS, yS, zS, linewidth=7, color='black')
		else:
...
def retarget():
	...
	first = inputs3d[0]
	plot_3d(ax, bones, first, "black")
	h = Human(1.8, "cpu", "h36m")
	model_1 = h.update_pose(r)
	plot_3d(ax, bones, model_1.detach().cpu().numpy())
	# plt.title("Retargeted human, azim=60", fontsize=35)
	...
	# plt.title("Retargeted human, azim=-140", fontsize=35)
	plt.tight_layout()
	plt.show()
