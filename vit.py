...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        print("")
        print(x.shape) # (1,3,384,384)
        emb = self.embedding(x)     
        print(emb.shape) # (1, 768, 24, 24)
        emb = emb.permute(0, 2, 3, 1)
        b, h, w, c = emb.shape # 1, 24, 24, 768
        emb = emb.reshape(b, h * w, c) 

        # prepend class token
        cls_token = self.cls_token.repeat(b, 1, 1)
        emb = torch.cat([cls_token, emb], dim=1) # 1, 576+1, 768
        ...
        # classifier
        logits = self.classifier(feat[:, 0]) # 768 -> 1000
